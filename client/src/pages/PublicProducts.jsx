...
import { getCategoriesAndBrands, getCategoriesAndBrandsScoped, searchProducts } from '../services/productService';
...
const loadProducts = useCallback(async (opts = {}, replace = false) => {
  try {
...
// getProductCategoryPath removed (unused)
                ...
                <div style={{ padding: 16, flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', borderTop: viewMode === 'list' ? 'none' : '1px solid #f0f0f0', paddingTop: viewMode === 'list' ? 0 : 12 }}>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      {info.discounted && <span style={{ textDecoration: 'line-through', color: '#999', fontSize: 13 }}>{formatPrice(info.original)}</span>}
                      <span style={{ fontSize: 18, fontWeight: 800, color: info.isOffer || info.isSale ? '#d32' : '#111' }}>
                        {formatPrice(info.display)}
                      </span>
                    </div>
