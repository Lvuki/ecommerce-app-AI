          ...
          {products.map(p => (
            <div key={p.id} style={{ border: '1px solid #eee', borderRadius: 10, overflow: 'hidden', background: '#fff' }}>
              ...
              <div style={{ padding: 12 }}>
                <div style={{ fontWeight: 600 }}><Link to={`/products/${p.id}`} style={{ color: 'inherit', textDecoration: 'none' }}>{p.name}</Link></div>
                ...
                <div style={{ marginTop: 10 }}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={async () => { try { const out = await wishlistService.toggleItem({ id: p.id, name: p.name, image: p.image, price: (p.offerPrice && Number(p.offerPrice) > 0) ? p.offerPrice : p.price }); const present = (out || []).find(i => String(i.id) === String(p.id)); alert(present ? 'Added to wishlist' : 'Removed from wishlist'); } catch (err) { console.error(err); alert('Failed to update wishlist'); } }} style={{ background: '#fff', border: '1px solid #e6e6e6', padding: '6px 8px', borderRadius: 6, fontSize: 13, cursor: 'pointer' }}>♡ Wishlist</button>
                  </div>
                </div>
              </div>
            </div>
