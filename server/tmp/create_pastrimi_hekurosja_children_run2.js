...
(async function main(){
  try {
    const rootName = 'PASTRIMI & HEKUROSJA';
    const root = await findOrCreateCategory(rootName, null);

    const children = ['FSHESA','HEKURA'];
    const created = {};
    ...
    const level2 = {
      'FSHESA': [
        'AKSESORE FSHESE','FSHESE','FSHESE DORE','FSHESE ROBOT','FSHESE VERTIKALE','FSHESE VERTIKALE ME AVULL'
      ],
      'HEKURA': [
        'AKSESORE HEKURI','HEKUR','HEKUR ME DEPOZITE','HEKUR UDHETIMI','HEKUR VERTIKAL'
      ]
    };
    ...
    for (const parentName of Object.keys(level2)) {
      ...
      for (const sub of level2[parentName]) { await findOrCreateCategory(sub, parent.id); }
    }

    console.log('Done creating PASTRIMI & HEKUROSJA children and level-2 children.');
    process.exit(0);
  } catch (err) {
    console.error('Error in create_pastrimi_hekurosja_children_run:', err);
    process.exit(2);
  }
})();
